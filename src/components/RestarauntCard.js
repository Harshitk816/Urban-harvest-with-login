    ...
    <div data-testid="resCard" className="shadow-2xl rounded-lg res-card mx-auto my-5 p-2 w-[200px] 2xs:max-xs:w-[260px] sm:w-[190px] bg-white hover:bg-gray-50 dark:bg-white
       transition-transform duration-300 transform hover:scale-105 " >
        <div className="relative drop-shadow-md"> 
            <img alt="res-logo" className="res-logo shadow-md rounded-lg" src={CDN_URL+cloudinaryImageId}></img>
            <label className=" absolute drop-shadow-md bottom-2 left-2 text-xs bg-white pe-2 py-1 rounded-2xl">⭐{avgRating}</label>
        </div>
        <div className="p-3 drop-shadow-md">
            <h3 className="font-bold py-2 drop-shadow-md text-lg truncate">{name}</h3>
            <h4 className=" text-sm drop-shadow-md py-1 truncate">{cuisines.join(", ")}</h4>
            <h4 className="text-sm py-1 flex">
                ...
                <button className="border border-solid border-[#4e7e6e] font-bold text-[#4e7e6e] px-2 mt-2 rounded-3xl hover:bg-slate-200 transition duration-500">View</button>
        </div>
    </div>
    </div>
)}
